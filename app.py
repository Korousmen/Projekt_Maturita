...
def register_user():
    ...
    if request.method == 'POST':  
        ...
        if not username or not email or not password:
            return "Všechna pole musí být vyplněna", 400

        password_hash = bcrypt.hashpw(password.encode('utf-8'), bcrypt.gensalt()).hex()
        ...
        try:
            ...
            cur.execute("INSERT INTO uziv (username, email, password, user_type_id) VALUES (%s, %s, %s, 2)",
                        (username, email, password_hash))
            connection.commit()
            connection.close()

            flash("Registrace úspěšná! Nyní se můžete přihlásit.", "success")
            return redirect(url_for('login_page'))  # Přesměrování na přihlášení
        except mysql.connector.Error as e:
            return f"Chyba databáze: {e}", 500
