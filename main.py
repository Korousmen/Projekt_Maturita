...
def insert_regr_user(username, email, password):
    try:
        ...
        print('Uživatel úspěšně přihlášen')

def get_hesh_from_database(email):
    try:
        connection = psycopg2.connect(
            dbname='projekt',
            user='postgres',
        ...
        cur = connection.cursor()
        query = '''SELECT password FROM uziv WHERE email=(%s)'''
...
def login_authentication(username, password, email):
    try:
        hash = get_hesh_from_database(email)
        ...
        result_label['text'] = 'Došlo k vhybe při ověřování '
...
email_login_label.grid(row=7, column=0)


email_login_entry = Entry()
email_login_entry.grid(row=7, column=1)
...
login_button = Button(text='Přihlásit se',
                      command=lambda: login_authentication(username_login_entry.get() ,password_login_entry.get(), email_login_entry.get()))
